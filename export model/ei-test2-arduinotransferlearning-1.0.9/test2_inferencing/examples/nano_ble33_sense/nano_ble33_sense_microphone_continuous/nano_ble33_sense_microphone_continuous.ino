// Added counter for "water flow" label detections > 0.8
#include <PDM.h>
...
static int water_flow_count = 0; // Count for "water flow" detections
...
void loop() {
    ...
    if (++print_results >= (EI_CLASSIFIER_SLICES_PER_MODEL_WINDOW)) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            ...
            if (strcmp(result.classification[ix].label, "water flow") == 0 && result.classification[ix].value > 0.8f) {
                water_flow_count++;
                ei_printf("\n>>> Water flow detected! Count: %d\n", water_flow_count);
            }
        }
...
static bool microphone_inference_record(void) {
    if (inference.buf_ready == 1) {
        ei_printf("Buffer overrun, decrease slices per model window\n");
        return false;
    }
