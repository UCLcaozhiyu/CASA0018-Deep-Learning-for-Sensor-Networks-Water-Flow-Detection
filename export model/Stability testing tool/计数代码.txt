...
static int water_flow_high_count = 0;
...
void loop() {
    ...
    if (++print_results >= EI_CLASSIFIER_SLICES_PER_MODEL_WINDOW) {
        ...
        for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
            ...
            if (strcmp(result.classification[ix].label, "water flow") == 0 &&
                result.classification[ix].value > 0.8f) {
                water_flow_high_count++;
            }
        }
...
#endif
        ei_printf("High confidence 'water flow' count: %d\n", water_flow_high_count);
